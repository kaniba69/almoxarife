...
import { ApiActionModal } from './components/ApiActionModal';
...
export default function App() {
  ...
  const handleQuickBidirectionalSync = async () => {
    try {
      setIsSyncingBidirectional(true);
      const res = await api.syncBidirectional();
      loadPublicData();
      if (currentUser) loadAdminData();
      addToast('success', res.message || 'Sincronização bidirecional realizada com sucesso!');
    } catch (err: any) {
      addToast('error', err.message || 'Falha ao executar sincronização bidirecional.');
    } finally {
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col font-['Plus_Jakarta_Sans',sans-serif]">
      ...
      <Navbar
        ...
        onQuickBidirectionalSync={handleQuickBidirectionalSync}
        isSyncingBidirectional={isSyncingBidirectional}
        isLoading={isLoading}
      />
